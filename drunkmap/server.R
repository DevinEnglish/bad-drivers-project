...
shinyServer(function(input, output) {
  ...
  output$drunk_overall <- renderPlotly({
    drunk_all <- ggplot(data= alcohol_levels, aes(x=year, y= Total, group=1)) +
      geom_line(color="steelblue4", size = 1.5)+
      geom_point(color = "tan1", size = 3.5) +
      theme_economist()+
      labs(title = paste("Number of drunk driving accidents by year"), x = "Year", y = "Number of accidents") + # accident type changes
      theme(plot.title = element_text(hjust = 0.5))
    ggplotly(drunk_all)
  })
